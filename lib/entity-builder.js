...
(function () {
    ...
    var _ = require('lodash'),
        Util = require('util'),
        StringUtils = require('smile/lib/utils/string-utils');
                ...
                if (entity.hasOwnProperty('one-to-one') || entity.hasOwnProperty('many-to-one') || entity.hasOwnProperty('many-to-many')) {
                    ...
                    if (entity.hasOwnProperty('one-to-one')) {
                        ...
                        for (i = 0; i < r.length; i++) {
                            ...
                            lines.push(StringUtils.capitalize(n) + '.hasOne(models.' + target + ', {');
                            lines.push('as : \'' + as + '\',');
                            lines.push('foreignKey : \'' + fKey + '\'');
                            lines.push('});');
                            lines.push(target + '.belongsTo(models.' + StringUtils.capitalize(n) + ');');
                        }
                    }
                    ...
                    if (entity.hasOwnProperty('many-to-many')) {
                        ...
                        for (i = 0; i < r.length; i++) {
                            target = r[i]['target-entity'][0];
                            lines.push(StringUtils.capitalize(n) + '.hasMany(models.' + target + ', {');
                            if (r[i].hasOwnProperty('join-table')) {
                                jTable = r[i]['join-table'][0]['name'][0];
                                as = r[i]['join-table'][0]['inverse-join-columns'][0]['join-column'][0]['name'][0];
                                lines.push('as: \'' + as + '\',');
                                lines.push('through: \'' + jTable + '\'');
                            }
                            lines.push('});');
